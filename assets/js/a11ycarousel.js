...
(function($) {
	$.fn.a11ycarousel = function(options) {
		...
		var animate = function(itemIndex) {
			...
			$(carouselContainer).find('ul.list li').each(function(){ $(this).removeClass('active').find('.course-one').removeAttr('tabindex'); });
			var listControl = $(carouselContainer).find('ul.list li')[itemIndex];
			var selectedControl = $(carouselContainer).find('.controls li button')[itemIndex];
			$(carouselContainer).find('ul.list li a').css('visibility','hidden');
			$(selectedControl).addClass('active');
			$(listControl).addClass('active').find('.course-one').attr('tabindex','0');
			$(listControl).find('a').css('visibility','visible');
		};
		...
		$(carouselContainer).prepend('<div class="controls"><div class="heading"><h2></h2></div></div>');
		...
		if (itemCount > 1) {
			...
			$(controlContainer).append('<input tabindex="0" type="button" class="toggle-play" value="' + (settings.autoPlay ? 'Pause' : 'Play') + '" title="' + (settings.autoPlay ? 'Pause' : 'Play') + ' the carousel" />');
			$(controlContainer).find('.toggle-play').click(togglePlay);
			$(controlContainer).append('<ul class="nav-bullets"></ul>').css({ 'z-index': 1000, position: 'absolute' });
			for (var i=0; i<itemCount; i++){
				$(controlContainer).find('ul').append('<li><button' + (i==0 ? ' class="active"' : '') + ' type="button" tabindex="0" value="' + (i+1) + '" title="Go to slide ' + (i+1) + '" aria-label="Go to slide ' + (i+1) + '">' + (i+1) + '</button></li>');
			}
